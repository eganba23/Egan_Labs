...
Here is a link to an archvied version of the assignment: https://web.archive.org/web/20250414135239/https://datascience4psych.github.io/DataScience4Psych/lab09alt.html#part-4-understanding-the-sources-of-bias
...
```{r risk score to recidivism visual,error = TRUE}
...
```{r compas accuracy, warning = FALSE,error = TRUE}
...
```{r accuracy calcuation,error = TRUE}
...
```{r false positive accuracy,error = TRUE}
...
```{r false negative accuracy,error = TRUE}
...
```{r visualize FP and FN,error = TRUE}
...
```{r prior and risk score visual, fig.height=8, fig.width=10,error = TRUE}
...
```{r other prior and risk score visual,error = TRUE}
...
```{r stretch other prior and risk score visual,error = TRUE}
...
```{r extra point,error = TRUE}
COMPAS_race_graph
```

##### Question 19 - other variables of interest

```{r COMPAS and other variables,error = TRUE}
